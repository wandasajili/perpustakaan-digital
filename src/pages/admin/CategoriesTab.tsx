  ...
  return (
    <div className="space-y-12 animate-in fade-in duration-700 pb-20">
      <div className="flex flex-col xl:flex-row justify-between items-center gap-6">
        <div>
           <h2 className="text-4xl font-display font-bold text-white tracking-tight">Arsitektur Kategori</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {loading && categories.length === 0 ? (
          <div className="col-span-full py-32 text-center">
           ...
           <div className="col-span-full admin-card p-24 text-center border-dashed border-2 border-slate-800">
          ...
          >
            ...
            <div className="w-16 h-16 bg-slate-950 border border-slate-800 text-indigo-400 rounded-2xl flex items-center justify-center mb-8 shadow-inner group-hover:border-indigo-500/50 group-hover:bg-indigo-500/5 transition-all">
              <Layers className="w-8 h-8" />
            </div>
            <h4 className="text-2xl font-display font-bold text-white mb-3 group-hover:text-indigo-400 transition-colors uppercase tracking-tight">{cat.name}</h4>
            ...
            <div className="flex gap-3 mt-auto">
              <button 
                ...
                className="flex-1 py-4 bg-slate-950 border border-slate-800 text-slate-400 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-slate-800 hover:text-white transition-all shadow-xl"
              >
                Modifikasi
              </button>
              <button 
                onClick={() => handleDelete(cat.id)}
                className="flex-1 py-4 bg-slate-950 border border-slate-800 text-slate-600 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-rose-500/10 hover:text-rose-500 hover:border-rose-500/30 transition-all shadow-xl"
              >
                Eliminasi
              </button>
            </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-slate-950/80 backdrop-blur-md">
            <motion.div
              ...
              className="bg-slate-900 w-full max-w-xl rounded-[48px] p-12 shadow-[0_0_100px_rgba(0,0,0,0.5)] border border-slate-800"
            >
              <div className="flex justify-between items-center mb-12">
                <div>
                  <h3 className="text-3xl font-display font-bold text-white tracking-tight leading-none">{editingCat ? 'Modifikasi Struktur' : 'Klasifikasi Baru'}</h3>
                  <p className="text-slate-500 font-medium text-sm mt-3">Definisikan skema klasifikasi secara presisi.</p>
                </div>
                <button 
                  onClick={() => setIsModalOpen(false)} 
                  className="w-14 h-14 flex items-center justify-center rounded-2xl bg-slate-950 border border-slate-800 hover:bg-slate-800 transition-all text-slate-400"
                >
                   <X className="w-7 h-7" />
                </button>
              </div>
              <form onSubmit={handleSubmit(onSubmit)} className="space-y-10">
                <div className="space-y-3">
                  ...
                  <input {...register('name')} className="w-full px-8 py-5 bg-slate-950 border border-slate-800 focus:border-indigo-500/50 text-white rounded-3xl outline-hidden font-bold transition-all placeholder:text-slate-800" placeholder="e.g. Metafisika & Filosofi" />
                </div>
                <div className="space-y-3">
                  <label className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] ml-2">Deskripsi Semantik</label>
                  <textarea {...register('description')} rows={5} className="w-full px-8 py-6 bg-slate-950 border border-slate-800 focus:border-indigo-500/50 text-white rounded-3xl outline-hidden font-bold transition-all resize-none placeholder:text-slate-800" placeholder="Uraikan esensi dari klasifikasi ini..." />
                </div>
